...
class Weather:
    ...
    def df(self, location=None, lib=None):
        ...
        # )
        point = location #(to_shape(location.point).x, to_shape(location.point).y)
        ...
        # drop duplicates
        for k in series:
            series[k] = series[k][~series[k].index.duplicated()]
            series[k].sort_index(inplace=True)
        ...
        if lib == "windpowerlib":
            ...
            series[("roughness_length", 0.0)] = series[
                ("roughness_length", 0.0)][
                ~series[("roughness_length", 0.0)].index.duplicated()]
            series[("roughness_length", 0.0)].sort_index(inplace=True)
            series.update({k: series[k][index] for k in series})

        index = series[list(series.keys())[0]].index
